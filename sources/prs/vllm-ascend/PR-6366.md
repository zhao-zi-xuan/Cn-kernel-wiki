...
> From the PR body (auto Summary above truncates at 400 chars). `measurement:
> source-reported`. The fused op reduces the KV-cache layout transpose cost to
> **0.24 ms from 7 ms in a unit test on 910C**, replacing the previous
> `npu_paged_cache_load + transpose + _npu_reshape_and_cache` sequence.
